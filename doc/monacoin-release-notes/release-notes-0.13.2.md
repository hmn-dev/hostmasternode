Hostmasternode Core version 0.13.2 is now available from:

  <https://download.hostmasternode.com/hostmasternode-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/hostmasternodeproject/hostmasternode/issues>
...
updates will be released anymore. Without security updates, using a hostmasternode
...
ECDSA signatures inside Hostmasternode transactions now use validation using
...
Previous versions of Hostmasternode Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Hostmasternode Core was to raise the default minimum
relay fee.

Hostmasternode Core 0.13.2 will have a strict maximum size on the mempool. The
...
Hostmasternode Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running hostmasternoded without having to do any manual configuration.
...
Hostmasternode Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Hostmasternode Core will relay transactions with
...
entirely in Hostmasternode Core 0.13, and it is at this time undecided whether the
...
Hostmasternode Core automatically creates a hidden service to listen on, without
manual configuration. Hostmasternode Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Hostmasternode Core is listening, and
...
Hostmasternoded can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Hostmasternode
Core to use floating fees. By default, Hostmasternode Core will use floating
fees.
...
(default: `0.0002` HMN/kB).

At all times, Hostmasternode Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HMN.
Furthermore, Hostmasternode Core will never create transactions paying less than
...
`hostmasternode.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /hostmasternoderpc>
        ...
        # optional bypass hostmasternoded rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< hostmasternoderpc:<password>
    </Location>
...
Restarting hostmasternoded will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in hostmasternode.conf
...
hostmasternode-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/hostmasternode-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Hostmasternode Core code base has
...
- `hostmasternode-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `hostmasternode-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Hostmasternode scripting system that in combination with
...
Hostmasternode transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Hostmasternode Core.
...
The two phases are distinct in the Hostmasternode-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Hostmasternode wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Hostmasternode.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Hostmasternode block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of hostmasternodes they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of hostmasternodes being spent is exactly
...
- **Increased security for multisig:** Hostmasternode addresses (both P2PKH addresses
  ...
  believed to be provided by Hostmasternode's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Hostmasternode's current ledger (called the UTXO set),
  ...
  note that Hostmasternode Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Hostmasternode users to individually opt-in to almost any change in the Hostmasternode
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Hostmasternode Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Hostmasternode
client to Hostmasternode Core 0.13.2 to signal for segwit support. When a
...
Since Hostmasternode Core 0.10.0, nodes have defaulted to only relaying and
...
- `hostmasternode-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `hostmasternode-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
