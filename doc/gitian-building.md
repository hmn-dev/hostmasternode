...
*Setup instructions for a Gitian build of Hostmasternode Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Hostmasternode
...
- [Building Hostmasternode Core](#building-hostmasternode-core)
...
and the hostmasternode git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hostmasternode/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url hostmasternode=${BTCPATH},signature=${SIGPATH} ../hostmasternode/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hostmasternode-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hostmasternode-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hostmasternode-osx-build.assert
```
