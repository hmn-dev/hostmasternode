...
// Copyright (c) 2013-2014 Hostmasternode Developers
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Hostmasternode: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    bnOld = bnNew;
    // Hostmasternode: intermediate uint256 can overflow by 1 bit
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    bnTarget.SetCompact(504365040, &fNegative, &fOverflow);


    if (bnTarget > UintToArith256(params.powLimit)) {
   	 std::cout <<  " bnTarget > UintToArith256(params.powLimit)  " << bnTarget.ToString() << " > "  << UintToArith256(params.powLimit).ToString() << " \n";
       	return false;
   }
    // Check range
    ...
    if (UintToArith256(hash) > bnTarget) {
    	 std::cout <<  " UintToArith256(hash) > bnTarget " << UintToArith256(hash).ToString() << " > "  << bnTarget.ToString() << " \n";
    	return false;
    }
    return true;
}
